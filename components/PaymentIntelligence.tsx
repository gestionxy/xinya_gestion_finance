...
export const PaymentIntelligence: React.FC<Props> = ({ forecastData, historyData, lang, standalone = false }) => {
                        ...
                        {paidHistoryList.length === 0 && (
                            <tr>
                                <td colSpan={7} className="px-4 py-8 text-center text-gray-500 italic">
                                    {filterValue ? t.alerts.noData : t.table.searchCompany}
                                </td>
                            </tr>
                        )}
                    </tbody>
                    ...
                    {checkSearchList.length === 0 && (
                        <tr>
                            <td colSpan={6} className="px-4 py-8 text-center text-gray-500 italic">
                                {filterValue ? t.alerts.noData : t.table.searchCheck}
                            </td>
                        </tr>
                    )}
                </tbody>
    ...
    return (
        <div className="space-y-6">
            {/* Expander for Detailed Tables */}
            <div className="border border-scifi-border rounded-xl overflow-hidden bg-[#0f172a]/50">
                <button
                    onClick={() => setExpanded(!expanded)}
                    className="w-full flex items-center justify-between p-4 bg-scifi-bg hover:bg-scifi-primary/10 transition-colors"
                >
                    <div className="flex items-center gap-2 text-scifi-primary font-bold">
                ...
                {expanded && (
                    <div className="p-4 border-t border-scifi-border animate-in slide-in-from-top-2">
                        {/* View Switcher */}
