...
export const UnpaidDeptChart: React.FC<DeptProps> = ({ data }) => {
    ...
    <div className="h-[500px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
          ...
          <Bar dataKey="amount" name="Unpaid Amount" radius={[4, 4, 0, 0]}>
             {chartData.map((entry, index) => (
                // Gradient Orange scale for Departments
                <Cell key={`cell-${index}`} fill={`hsl(25, 90%, ${50 + (index * 4)}%)`} />
             ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
