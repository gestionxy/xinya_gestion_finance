...
export const ForecastChart: React.FC<ForecastProps> = ({ data, view, selectedDept }) => {
    ...
    <div className="h-[450px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
          ...
          <Bar dataKey="amount" name="Due Amount" radius={[4, 4, 0, 0]}>
             {chartData.map((entry, index) => (
                <Cell key={`cell-${index}`} fill={`hsl(200, 90%, ${50 + (index * 3)}%)`} />
             ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
